  ...
  try {
    ...
    if (dsAttachments != null && dsAttachments.getRowsCount() > 0) {
      ...
      for (var j = 1; j < dsAttachments.getRowsCount(); j++) {
        ...
        var constraintDocId = DatasetFactory.createConstraint(
          "documentPK.documentId",
          documentId,
          documentId,
          ConstraintType.MUST,
        );
        var constraintVersion = DatasetFactory.createConstraint(
          "documentPK.version",
          version,
          version,
          ConstraintType.MUST,
        ...
        var dsDocument = DatasetFactory.getDataset(
          "document",
          null,
          [constraintDocId, constraintVersion],
          null,
        );
        ...
        if (dsDocument != null && dsDocument.getRowsCount() > 0) {
          fileName = dsDocument.getValue(0, "phisicalFile");
        }

        try {
          var bytes = docService.getDocumentContentAsBytes(
            parseInt(documentId),
          );
          if (bytes != null) {
        ...
        dataset.addRow([documentId, fileName, base64]);
      }
    }
  } catch (error) {
